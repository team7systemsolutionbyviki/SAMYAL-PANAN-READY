...
onAuthStateChanged(auth, async (user) => {
    ...
    if (session === 'viki_super_admin') {
        ...
        const expectedEmail = "viki@sfc.com";
        const expectedPass = "VIKI1101";

        const syncAdminProfile = async (currentUser) => {
            try {
                ...
                if (!userDocSnap.exists() || userDocSnap.data().role !== 'admin') {
                    ...
                    console.log("Super Admin Firestore profile synced to admin!");
                }
            } catch (err) {
                console.warn("Super Admin Firestore profile sync failed:", err);
            }
        };
        ...
        if (user && user.email === expectedEmail) {
            ...
            try {
                ...
                console.warn("Super admin background auth sync failed:", err);
                initAdminSystem();
            }
        }
        return;
    ...
    if (session === 'hari_admin') {
        const expectedEmail = "hari@sfc.com";
        const expectedPass = "HARI654321";
        ...
        const syncAdminProfile = async (currentUser) => {
            try {
                ...
                if (!userDocSnap.exists() || userDocSnap.data().role !== 'admin') {
                    ...
                    console.log("Store Admin Firestore profile synced to admin!");
                }
            } catch (err) {
                console.warn("Store Admin Firestore profile sync failed:", err);
            }
        };
        ...
        if (user && user.email === expectedEmail) {
            ...
            try {
                ...
                console.warn("Store admin background auth sync failed:", err);
                initAdminSystem();
            }
        }
        return;
...
let isAdminSystemInitialized = false;
function initAdminSystem() {
    if (isAdminSystemInitialized) return;
    isAdminSystemInitialized = true;
    ...
    // Sidebar navigation click handling with debug logs
    sidebarNav.addEventListener('click', (e) => {
        const btn = e.target.closest('.nav-item');
        console.log('Nav click target:', e.target);
        ...
        // Draw charts if dashboard is shown!
        if (view === 'dashboard') {
            drawDashboardCharts();
        }
    });
    ...
    onSnapshot(doc(db, "settings", "shop"), (snap) => {
        if (snap.exists()) {
            ...
            if (!shopSettings.schedule) {
                ...
                setDoc(doc(db, "settings", "shop"), shopSettings).catch(err => {
                    console.warn("Could not save migrated shop settings to Firestore:", err);
                });
            }
            ...
            updateShopSettingsUI();
            setDoc(doc(db, "settings", "shop"), initial).catch(err => {
                console.warn("Could not save initial shop settings to Firestore:", err);
            });
        }
    });
    ...
    // 13. Admins Listener (Super Admin Only)
    if (localStorage.getItem('admin_session') === 'viki_super_admin') {
...
function updateShopSettingsUI() {
    ...
    if (container && template && shopSettings.schedule) {
        ...
        DAYS_OF_WEEK.forEach(day => {
            ...
            const openVal = dayData?.open || '09:00';
            const closeVal = dayData?.close || '22:00';
            const isClosedVal = dayData ? !!dayData.isClosed : false;

            openInp.value = openVal;
            closeInp.value = closeVal;
            closedCheck.checked = isClosedVal;

            // Toggle opacity if closed
            const row = clone.querySelector('.day-row');
            if (isClosedVal) row.style.opacity = '0.5';
    ...
    // Populate the WhatsApp number field
    const waInput = document.getElementById('shop-whatsapp-number');
    if (waInput && shopSettings.whatsappNumber) {
        waInput.value = shopSettings.whatsappNumber;
    }
...
if (shopHoursForm) {
    shopHoursForm.onsubmit = async (e) => {
        ...
        // Read WhatsApp number from the new field
        const waInput = document.getElementById('shop-whatsapp-number');
        const whatsappNumber = waInput ? waInput.value.trim().replace(/[^0-9]/g, '') : '';
        ...
        try {
            await setDoc(doc(db, "settings", "shop"), { schedule: newSchedule, whatsappNumber });
            showToast("✅ Schedule & WhatsApp number saved!", "success");
        } catch (err) {
...
// Duplicate navigation handler removed - using the earlier enhanced handler with debug logs
// --- Dashboard Logic ---
...
function renderProducts(products) {
    ...
    sorted.forEach(p => {
        ...
        // Pricing or variants rendering
        let priceContent = '';
        if (p.variants && p.variants.length > 0) {
            priceContent = '<ul style="margin:0; padding-left:12px; font-size:0.8rem; list-style:none; text-align:left;">';
            p.variants.forEach(v => {
                priceContent += `<li>• <strong>${v.name}:</strong> ₹${v.price}</li>`;
            });
            priceContent += '</ul>';
        } else {
            priceContent = `₹${p.price || 0}${p.unit === 'GRM' ? '/100g' : ''}`;
        }

        // Spice levels rendering
        let spiceChilis = '-';
        if (p.spiceLevel) {
            const sl = p.spiceLevel.toLowerCase();
            let chilisCount = 1;
            if (sl.includes('extra')) chilisCount = 4;
            else if (sl.includes('spicy')) chilisCount = 3;
            else if (sl.includes('medium')) chilisCount = 2;
            spiceChilis = '<span style="font-size: 1.1rem; vertical-align: middle;">' + '🌶️'.repeat(chilisCount) + '</span>' + ` <span style="font-size:0.75rem; color:var(--text-gray);">(${p.spiceLevel})</span>`;
        }

        // Stock levels rendering
        let stockBadge = '';
        if (p.stock === undefined || p.stock === null || p.stock === '') {
            stockBadge = '<span class="status-badge" style="background:rgba(16,185,129,0.15); color:#10b981; border: 1px solid rgba(16,185,129,0.3);">Unlimited</span>';
        } else if (p.stock <= 0) {
            stockBadge = `<span class="status-badge" style="background:rgba(239,68,68,0.15); color:#ef4444; border: 1px solid rgba(239,68,68,0.3);">Sold Out (${p.stock})</span>`;
        } else if (p.stock <= 3) {
            stockBadge = `<span class="status-badge" style="background:rgba(245,158,11,0.15); color:#f59e0b; border: 1px solid rgba(245,158,11,0.3);">Low (${p.stock})</span>`;
        } else {
            stockBadge = `<span class="status-badge" style="background:rgba(16,185,129,0.15); color:#10b981; border: 1px solid rgba(16,185,129,0.3);">${p.stock}</span>`;
        }

        tr.innerHTML = `
            <td><img src="${p.imageUrl || 'https://images.unsplash.com/photo-1544025162-d76694265947?q=80&w=600&auto=format&fit=crop'}" class="td-img" style="border-radius:4px; object-fit:cover; width:50px; height:50px;"></td>
            <td><strong>${p.name}</strong>${p.subCategory ? `<br><span style="font-size:0.75rem; color:var(--text-gray);">${p.subCategory}</span>` : ''}</td>
            <td><span class="status-badge" style="background:rgba(255,255,255,0.05); color:var(--text-light);">${p.category}</span></td>
            <td>${priceContent}</td>
            <td><span class="status-badge" style="background:rgba(255,255,255,0.05); color:var(--text-gray);">${p.unit || 'PCS'}</span></td>
            <td>${spiceChilis}</td>
            <td><span style="font-size:0.85rem; color:var(--text-light);">${p.cookingTime || '-'}</span></td>
            <td>${stockBadge}</td>
            <td>
...
document.getElementById('open-product-modal').onclick = () => {
    ...
    const hasVariantsCheckbox = document.getElementById('p-has-variants');
    ...
    if (singlePriceWrap) singlePriceWrap.style.display = 'block';
    if (pPriceInput) pPriceInput.setAttribute('required', '');

    // Reset details
    const pSubcategory = document.getElementById('p-subcategory');
    if (pSubcategory) pSubcategory.value = '';
    const pSpice = document.getElementById('p-spice');
    if (pSpice) pSpice.value = '';
    const pCookingTime = document.getElementById('p-cooking-time');
    if (pCookingTime) pCookingTime.value = '';
...
productForm.onsubmit = async (e) => {
    ...
    const spiceLevel = document.getElementById('p-spice')?.value || '';
    const cookingTime = document.getElementById('p-cooking-time')?.value.trim() || '';
    const stockVal = document.getElementById('p-stock')?.value;
    const stock = (stockVal !== undefined && stockVal !== '') ? parseInt(stockVal) : null;
    ...
    try {
        if (hasVariants) {
            ...
            if (variants.length === 0) {
                showToast("Please add at least one variant or disable variants", "error");
                btn.disabled = false;
                return;
            }
...
if (exportCsvBtn) {
    exportCsvBtn.onclick = () => {
        ...
        link.setAttribute("download", "samayal-panna-ready-inventory.csv");
...
window.openEditModal = (id) => {
    ...
    imgPreview.innerHTML = p.imageUrl ? `<img src="${p.imageUrl}">` : '';
    document.getElementById('modal-title').innerText = 'Edit Food Item';

    // Populate extra fields
    const pSubcategory = document.getElementById('p-subcategory');
    if (pSubcategory) pSubcategory.value = p.subCategory || '';
    const pSpice = document.getElementById('p-spice');
    if (pSpice) pSpice.value = p.spiceLevel || '';
    const pCookingTime = document.getElementById('p-cooking-time');
    if (pCookingTime) pCookingTime.value = p.cookingTime || '';
    const pStock = document.getElementById('p-stock');
    if (pStock) pStock.value = (p.stock !== undefined && p.stock !== null) ? p.stock : '';
    ...
    if (hasVariants) {
        ...
        if (pPriceInput) {
            pPriceInput.removeAttribute('required');
            pPriceInput.value = '';
        }
        p.variants.forEach(v => {
        ...
        if (singlePriceWrap) singlePriceWrap.style.display = 'block';
        if (pPriceInput) {
            pPriceInput.setAttribute('required', '');
            pPriceInput.value = p.price || '';
        }
    }
...
function renderOrders(orders) {
    ...
    sorted.forEach(o => {
        ...
        const itemsList = o.items.map(i => {
            const variantStr = i.variantName ? `(${i.variantName})` : (i.unit ? `(${i.unit})` : '');
            const spiceStr = i.spiceLevel ? ` - Spice: ${i.spiceLevel}` : '';
            return `• ${i.name} ${variantStr}${spiceStr} [x${i.quantity}]`;
        }).join('<br>');
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="cust-info">
                    ...
                    ${o.address ? `<br><small>🏠 <strong>Address:</strong> ${o.address}</small>` : ''}
                    ...
                    ${o.notes ? `<div style="margin-top:6px;font-size:0.82rem;color:var(--text-muted);background:rgba(255,193,7,0.12);border-left:3px solid #ffc107;padding:5px 8px;border-radius:4px;"><strong>📝 Special Instructions:</strong><br>${o.notes}</div>` : ''}
                </div>
            </td>
            <td>
                ₹${o.totalPrice}
                ${o.discount > 0 ? `<br><small style="color:var(--error); font-size:0.75rem;">(Disc: -₹${o.discount})</small>` : ''}
            </td>
...
window.sendWA = (id) => {
    ...
    let msg = `🍽️ *SAMAYAL PANNA READY - ORDER BILL*\n\n`;
    ...
    msg += `❤ *Thank you for choosing SAMAYAL PANNA READY!* ❤\n`;
...
window.addToPOSCart = (product) => {
    if (product.unit === 'GRM' || product.unit === 'PCS') {
        pendingGrmProduct = product;
        document.getElementById('qty-modal-product').innerText = product.name;
        
        const label = document.getElementById('qty-modal-label');
        const title = document.getElementById('qty-modal-title');
        const input = document.getElementById('qty-input');

        if (product.unit === 'GRM') {
            title.innerText = "Enter Weight";
            label.innerText = "Grams (GRM)";
            input.value = 100;
        } else {
            title.innerText = "Enter Quantity";
            label.innerText = "Pieces (PCS)";
            input.value = 1;
        }
...
function renderPOSCart() {
    ...
    const discount = parseFloat(posDiscountEl?.value) || 0;
...
if (posDiscountEl) posDiscountEl.oninput = renderPOSCart;
...
if (btnCompletePOS) {
    btnCompletePOS.onclick = async () => {
        ...
        try {
            ...
            if (posDiscountEl) posDiscountEl.value = '';
            renderPOSCart();
        } catch (err) {
...
async function generatePeriodReport() {
    ...
    const totalDiscount = filteredOrders.reduce((acc, o) => acc + (parseFloat(o.discount) || 0), 0);
    ...
    if (document.getElementById('report-discounts')) {
        document.getElementById('report-discounts').innerText = `₹${Math.round(totalDiscount)}`;
    }
...
function exportReportCSV() {
    ...
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `SAMAYAL_PANNA_READY_Report_${start}_to_${end}.csv`);
    document.body.appendChild(link);
    link.click();
...
function renderAdmins() {
    ...
    allAdmins.forEach(adm => {
        if (adm.email === 'viki@sfc.com') return; // Hide Super Admin VIKI
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><strong>${adm.name || 'Admin'}</strong></td>
            <td>${adm.email || 'N/A'}</td>
            <td>
                <button class="btn-action btn-delete" onclick="deleteAdmin('${adm.id}')">Remove</button>
            </td>
        `;
        body.appendChild(tr);
    });
...
const openAddAdminBtn = document.getElementById('open-add-admin');
const closeAdminModalBtn = document.getElementById('close-admin-modal');
const adminCreatorForm = document.getElementById('admin-creator-form');

if (openAddAdminBtn && adminModal) {
...
if (adminCreatorForm && adminModal) {
    adminCreatorForm.onsubmit = async (e) => {
        e.preventDefault();
        const name = document.getElementById('a-name').value;
        const email = document.getElementById('a-email').value;
        const pass = document.getElementById('a-pass').value;

        showToast("Registering admin...", "info");
        ...
        try {
            ...
            await setDoc(doc(tempDb, "users", userCred.user.uid), {
...
function addVariantRow(name = '', price = '') {
    ...
    row.innerHTML = `
        <input type="text" class="v-name" placeholder="Name (e.g. 500g)" value="${name}" required style="flex: 2; min-width: 0; background: rgba(255,255,255,0.03); color: var(--text-light); border: 1px solid var(--border-glass); padding: 8px; border-radius: var(--radius-sm);">
        <input type="number" class="v-price" placeholder="Price (₹)" value="${price}" required min="0" step="0.01" style="flex: 1; min-width: 0; background: rgba(255,255,255,0.03); color: var(--text-light); border: 1px solid var(--border-glass); padding: 8px; border-radius: var(--radius-sm);">
        <button type="button" class="btn-action btn-delete remove-variant-btn" style="background: var(--error); margin: 0; padding: 10px; height: auto; display: flex; align-items: center; justify-content: center;">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
    `;

    row.querySelector('.remove-variant-btn').onclick = () => {
        row.remove();
    };
...
// Call setup once initialized
setupVariantsListeners();

// --- HTML5 Canvas Charts Implementation ---
function drawDashboardCharts() {
    const canvas1 = document.getElementById('chart-order-volume');
    const canvas2 = document.getElementById('chart-revenue-expenses');
    ...
    if (rect1.width === 0 || rect2.width === 0) {
        // Elements are hidden (dashboard not visible), skip drawing
        // If dashboard view is not hidden, schedule a retry in 100ms when layout is computed.
    ...
    const w1 = rect1.width || 320;
    const h1 = rect1.height || 250;
    const w2 = rect2.width || 320;
    const h2 = rect2.height || 250;

    canvas1.width = w1 * dpr;
    canvas1.height = h1 * dpr;
    canvas1.style.width = w1 + 'px';
    canvas1.style.height = h1 + 'px';

    canvas2.width = w2 * dpr;
    canvas2.height = h2 * dpr;
    canvas2.style.width = w2 + 'px';
    ...
    // Compute dates for the last 7 days
    const dates = [];
    const labels = [];
    for (let i = 6; i >= 0; i--) {
        const d = new Date();
    ...
    allOrders.forEach(o => {
        ...
        for (let i = 0; i < 7; i++) {
            const d = dates[i];
            if (d.getFullYear() === oDate.getFullYear() &&
                d.getMonth() === oDate.getMonth() &&
                d.getDate() === oDate.getDate()) {
                orderCounts[i]++;
                revenues[i] += parseFloat(o.totalPrice) || 0;
                break;
            }
        }
    });
    ...
    allExpenses.forEach(ex => {
        ...
        for (let i = 0; i < 7; i++) {
            const d = dates[i];
            if (d.getFullYear() === exDate.getFullYear() &&
                d.getMonth() === exDate.getMonth() &&
                d.getDate() === exDate.getDate()) {
                expenses[i] += parseFloat(ex.amount) || 0;
                break;
            }
        }
    });

    drawSmoothLineChart(ctx1, w1, h1, labels, orderCounts);
    drawGroupedBarChart(ctx2, w2, h2, labels, revenues, expenses);
}

function drawSmoothLineChart(ctx, w, h, labels, data) {
    ...
    for (let i = 0; i <= ticks; i++) {
        ...
        const y = paddingTop + (1 - i / ticks) * chartHeight;
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const y = paddingTop + (1 - data[i] / yMax) * chartHeight;
        points.push({ x, y });
    }

    // Fill Gradient under Curve
    const fillGradient = ctx.createLinearGradient(0, paddingTop, 0, paddingTop + chartHeight);
    fillGradient.addColorStop(0, 'rgba(255, 77, 36, 0.25)');
    fillGradient.addColorStop(1, 'rgba(255, 77, 36, 0.0)');
...
function drawGroupedBarChart(ctx, w, h, labels, revenues, expenses) {
    ctx.clearRect(0, 0, w, h);
    ...
    const maxVal = Math.max(...revenues, ...expenses, 500);
    ...
    const barWidth = Math.max(6, Math.min(12, groupWidth * 0.3));
    const gap = 2;

    for (let i = 0; i < 7; i++) {
        const xCenter = paddingLeft + (i + 0.5) * groupWidth;
        const xRev = xCenter - barWidth - gap / 2;
        const xExp = xCenter + gap / 2;

        const hRev = (revenues[i] / yMax) * chartHeight;
        const hExp = (expenses[i] / yMax) * chartHeight;
        ...
        // Draw Revenue Bar
        ctx.fillStyle = '#ff4d24';
        drawRoundedRect(ctx, xRev, yRev, barWidth, hRev, 2);

        // Draw Expense Bar
        ctx.fillStyle = '#ef4444';
        drawRoundedRect(ctx, xExp, yExp, barWidth, hExp, 2);
    }
    ...
    labels.forEach((label, i) => {
        const xCenter = paddingLeft + (i + 0.5) * groupWidth;
        ctx.fillText(label, xCenter, paddingTop + chartHeight + 8);
    });
    ...
    ctx.fillStyle = '#ff4d24';
    ctx.fillRect(w - 130, 5, 8, 8);
    ctx.fillStyle = '#9ca3af';
    ctx.fillText('Revenue', w - 85, 4);

    ctx.fillStyle = '#ef4444';
    ctx.fillRect(w - 75, 5, 8, 8);
    ctx.fillStyle = '#9ca3af';
    ctx.fillText('Expenses', w - 30, 4);
}
...
function renderReviews() {
    ...
    sorted.forEach(rev => {
        ...
        const dateStr = rev.timestamp ? new Date(rev.timestamp.seconds * 1000).toLocaleString() : 'Just now';

        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><strong>${productName}</strong><br><small style="color:var(--text-muted); font-size:0.75rem;">${rev.productId}</small></td>
            <td><strong>${rev.userName || 'Verified Buyer'}</strong><br><small style="color:var(--text-muted); font-size:0.75rem;">${rev.userId || 'N/A'}</small></td>
            <td style="color:var(--accent-gold); font-size:0.9rem;">${'⭐'.repeat(rev.rating)}</td>
            <td><div style="max-width:300px; overflow-wrap:break-word; font-size:0.85rem;">${rev.comment || 'No comment.'}</div></td>
            <td style="font-size:0.8rem; color:var(--text-muted);">${dateStr}</td>
            <td>
                <button class="btn-action btn-edit" onclick="editReview('${rev.id}')">Edit</button>
                <button class="btn-action btn-delete" onclick="deleteReview('${rev.id}')">Delete</button>
            </td>
        `;
        body.appendChild(tr);
    });
...
window.openAddReviewModal = () => {
    const select = document.getElementById('add-review-product');
    if (select) {
...
function setupReviewsListeners() {
    ...
    const closeEditReviewModalBtn = document.getElementById('close-edit-review-modal');
    const editReviewModal = document.getElementById('edit-review-modal');

    if (closeEditReviewModalBtn && editReviewModal) {
        closeEditReviewModalBtn.onclick = () => {
    ...
    if (openAddReviewBtn) {
        openAddReviewBtn.onclick = () => {
            openAddReviewModal();
        };
    }

    if (closeAddReviewModalBtn && addReviewModal) {
        closeAddReviewModalBtn.onclick = () => {
    ...
    if (addReviewForm) {
        addReviewForm.onsubmit = async (e) => {
            e.preventDefault();
            const productId = document.getElementById('add-review-product').value;
            const userName = document.getElementById('add-review-user').value.trim();
            const rating = parseInt(document.getElementById('add-review-rating').value);
            const comment = document.getElementById('add-review-comment').value.trim();
            ...
            try {
                ...
                addReviewForm.reset();
                if (addReviewModal) addReviewModal.classList.add('hidden');
            } catch (err) {
